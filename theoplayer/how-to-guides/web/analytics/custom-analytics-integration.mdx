...
- [Network](../network/introduction.mdx): to intercept HTTP(S) requests and responses. You should only use this API when you can't use [Service Workers](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API/Using_Service_Workers) to accomplish the same.
...
and our articles on [error handling](../miscellaneous/error/introduction.mdx).
...
Another useful "fake" event that you could track is the "video startup time", as discussed at [How to measure time-to-first-frame?](../player/track-first-playing-event.mdx).
