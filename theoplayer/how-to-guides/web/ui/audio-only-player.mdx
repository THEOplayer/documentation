---
sidebar_position: 14
sidebar_label: Make an audio-only player
---

# How to make an audio-only player on Web

This article tells you how to manipulate THEOplayer to make an audio-only interface, using only CSS and JavaScript.
...
Note: if you preload the source, hiding the time information before the player starts playing may not be needed, as the duration will be already available.
