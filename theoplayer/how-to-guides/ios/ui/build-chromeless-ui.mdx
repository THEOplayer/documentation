---
sidebar_position: 1
sidebar_label: Getting started
---

# Getting started with a chromeless UI on iOS and tvOS

import DocCardList from '@theme/DocCardList';
import Intro from '../../shared/ui/_build-chromeless-ui-intro.mdx';

<Intro />

## Create a chromeless player instance

The iOS and tvOS SDKs do not ship a default UI. A [`THEOplayer`](pathname:///theoplayer/v11/api-reference/ios/Classes/THEOplayer.html) instance renders video, and your application adds native controls on top.

```swift
let config = THEOplayerConfiguration(license: "your_license_string")
let player = THEOplayer(configuration: config)
```

The following guides refer to this `player` variable.

## Track player states

Before adding controls, learn how to [track player states](../player/track-player-states.mdx). Your UI should respond correctly when playback starts, pauses, buffers, ends, or encounters an error.

![Video Player States](../../../assets/img/player-video-player-states.png 'Video Player States')

## Next steps

<DocCardList
  items={[
    {
      type: 'link',
      label: 'Map UI components',
      description: 'Connect native controls and displays to the appropriate Player APIs.',
      href: '/theoplayer/how-to-guides/ios/ui/chromeless/mapping-components/',
    },
    {
      type: 'link',
      label: 'Explore the sample project',
      description: 'Review a basic custom UI implementation for the iOS SDK.',
      href: '/theoplayer/how-to-guides/ios/ui/chromeless/sample-code/',
    },
    {
      type: 'link',
      label: 'Add UX enhancements',
      description: 'Plan loading, poster, next-content, intro-skip, and advertising experiences.',
      href: '/theoplayer/how-to-guides/ios/ui/chromeless/ux-enhancements/',
    },
    {
      type: 'link',
      label: 'Handle errors',
      description: 'Make failures visible and actionable in your custom UI.',
      href: '/theoplayer/how-to-guides/ios/ui/chromeless/error-handling/',
    },
  ]}
/>
