...
Here's how to get started integrating the Conviva Connector with the THEO Roku SDK.
...
3. Then configure the connector by calling the configure method, passing the player instance and your Conviva customer key.
...
5. If you desire to monitor for CDN changes, you can optionally add a configuration for that after starting the session.
   `m.convivaConnector.callFunc("monitorCdnChanges", { mycdn: ["my-cdn.net"], theo: ["cdn.theoplayer.com"] })`
6. If the content metadata needs to change, you can update it by calling `setContentInfo`. This method accepts partial content metadata:
   `m.convivaConnector.callFunc( "setContentInfo", {assetName: "New Program"})`
7. When the video has stopped playing because it ended or the user exited, end the Conviva session
