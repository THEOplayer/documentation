...
| googleDaiProxy    | GoogleDAIProxyNode   | read              | roSGNode that exposes data from the internal Google DAI library.                                              |
...
The Google DAI proxy node has a field that can be observed to consume data generated by the internal Google DAI library. This way, an application can access information traditionally available via the callbacks for ad events on Google DAI.

| Property  | Type       | Access Permission | Description                                                                                  |
| --------- | ---------- | ----------------- | -------------------------------------------------------------------------------------------- |
| lastEvent | assocarray | read              | An associative array representing the ad from Google DAI with an additional `type` property. |
