...
| drmParams   | DRM Parameters | An alternative way to set parameters for DRM playback that follows the Roku Content Node's `drmParams`. _This attribute is only used when the `integration` attribute is not set._ |
