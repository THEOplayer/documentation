...
because its license (i.e. `'your_license_string'`) and library location (i.e. `'SDK-LIBRARY-LOCATION'`) contain invalid placeholder values.
...
Similar to the JavaScript and CSS library, you must swap out `'SDK-LIBRARY-LOCATION'` and `'your_license_string'`.
