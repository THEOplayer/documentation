...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="/path/to/node_modules/theoplayer/ui.css" />
  </head>
  <body>
    <div class="theoplayer-container video-js theoplayer-skin vjs-16-9"></div>
    <script src="/path/to/node_modules/theoplayer/THEOplayer.js"></script>
    <script>
...
because its license (i.e. `'your_license_string'`) and library location (i.e. `'/path/to/node_modules/theoplayer/'`) contain invalid placeholder values.
...
<details>
    <summary>While we do recommend getting your THEOplayer Web SDK through NPM, it is also possible to get yours through the THEOplayer portal.</summary>
    ...
    The following sections of this guide assume the SDK is downloaded through NPM. Instead of `'path/to/node_modules/theoplayer'`,
    you can use the SDK library URL in the <span className={styles.borderBlue}>blue rectangle</span>.
    For the license you can use the value in the <span className={styles.borderGreen}>green rectangle</span>.
</details>
...
In a previous step we downloaded the [THEOplayer npm package](#installing-theoplayer-web-sdk), now we'll include it in our HTML file.
...
The `src` attribute is the URI of the THEOplayer library that you wish to use. This could be something like: `'/path/to/node_modules/theoplayer/THEOplayer.js'`,
...
Similar to the JavaScript library, you must swap `/path/to/` with your actual path.
...
<script>
  ...
  var player = new THEOplayer.Player(element, {
    ...
    libraryLocation: "/path/to/node_modules/theoplayer/", // references folder containing your THEOplayer library files (theoplayer.p.js, THEOplayer.js, ...)
...
Similar to the JavaScript and CSS library, you must swap out `'/path/to/'` and `'your_license_string'`.
