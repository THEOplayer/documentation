...
## 1. Import the SDK[​](#1-import-the-sdk "Direct link to 1. Import the SDK")
...
## 2. Configure the audio session for playback.[​](#2-configure-the-audio-session-for-playback "Direct link to 2. Configure the audio session for playback.")
...
## 3. Create a subscriber[​](#3-create-a-subscriber "Direct link to 3. Create a subscriber")
...
## 4. Setup your credentials[​](#4-setup-your-credentials "Direct link to 4. Setup your credentials")
...
## 5. Subscribe to a stream[​](#5-subscribe-to-a-stream "Direct link to 5. Subscribe to a stream")

Subscribing a stream includes two steps.

### 5.1. Connect to the Millicast service[​](#51-connect-to-the-millicast-service "Direct link to 5.1. Connect to the Millicast service")
...
### 5.2. Subscribe with the preferences[​](#52-subscribe-with-the-preferences "Direct link to 5.2. Subscribe with the preferences")
...
## 6. Manage broadcast events[​](#6-manage-broadcast-events "Direct link to 6. Manage broadcast events")

When broadcast events occur, the SDK emits the appropriate event. There are multiple ways to receive those events, using AsyncStream's or by subscribing to Combine Publisher's. Those events can also be received by conforming to [MCSubscriberDelegate](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcsubscriberdelegate) delegate methods. Note: This listener has to be setup before calling the [subscribe(with:)](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcsubscriber/subscribe\(completionhandler:\)) method.

### 6.1. Receive Audio and Video tracks[​](#61-receive-audio-and-video-tracks "Direct link to 6.1. Receive Audio and Video tracks")
...
Receive newly added tracks by listening to subscriber's [rtsRemoteTrackAdded()](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcsubscriber/rtsremotetrackadded) stream. Newly added tracks can alternatively be received using [rtsRemoteTrackAddedPublisher](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcsubscriber/rtsremotetrackaddedpublisher) or the delegate method [subscriber(\_:didReceiveRTSRemoteTrack:)](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcsubscriberdelegate/subscriber\(_:didreceivertsremotetrack:\))

Define variables to store the tracks and renderers. Important: Audio and Video tracks of the same source will have the same [sourceId](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcrtsremotetrack/sourceid)
...
### 6.2. Listen to active/inactive state of tracks[​](#62-listen-to-activeinactive-state-of-tracks "Direct link to 6.2. Listen to active/inactive state of tracks")

Listen to the active and inactive state of a [MCRTSRemoteTrack](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcrtsremotetrack) using [activity()](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcrtsremotetrack/activity) stream. Alternatively receive this event using the combine publisher [activityPublisher()](https://millicast.github.io/doc/latest/apple/documentation/millicastsdk/mcrtsremotetrack/activitypublisher)
...
Similarly you can listen to active/inactive events of an audio track as below. Note: An audio track plays audio only when its enabled.
...
### 6.3. Receive layer information[​](#63-receive-layer-information "Direct link to 6.3. Receive layer information")
...
## 7. Listen to Websocket and Peer connection state changes[​](#7-listen-to-websocket-and-peer-connection-state-changes "Direct link to 7. Listen to Websocket and Peer connection state changes")
...
## 8. Render video on the UI[​](#8-render-video-on-the-ui "Direct link to 8. Render video on the UI")
...
## 9. Collecting RTC statistics[​](#9-collecting-rtc-statistics "Direct link to 9. Collecting RTC statistics")
...
## 10. Error handling[​](#10-error-handling "Direct link to 10. Error handling")
...
## 11. Unsubscribe the session[​](#11-unsubscribe-the-session "Direct link to 11. Unsubscribe the session")
