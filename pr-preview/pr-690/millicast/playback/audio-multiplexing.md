...
## Understanding Audio Multiplexing[​](#understanding-audio-multiplexing "Direct link to Understanding Audio Multiplexing")

To first understand Audio Multiplexing, we need to understand how the OptiView Real-time servers ingest feeds. When [Broadcasting](/documentation/pr-preview/pr-690/millicast/broadcast.md), the OptiView Real-time Streaming servers will assign the most recent published stream source as the "*Main Source*", a function that allows broadcasters to seamlessly overwrite streams or add [Redundant Streams](/documentation/pr-preview/pr-690/millicast/broadcast/redundant-ingest.md). For basic broadcasts, this is sufficient, for more advanced broadcasts OptiView Real-time Streaming provides two features that are exceptions:

1. Creating a [Multisource stream](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md) for [Multi-view](/documentation/pr-preview/pr-690/millicast/playback/multiview.md) experiences.
...
If you've reviewed the [Multi-source Broadcasting](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md) and [Multi-view](/documentation/pr-preview/pr-690/millicast/playback/multiview.md) documentation, you may be confused as to why you would use Audio Multiplexing instead of Multiview. Audio multiplexing allows audio sources to overlap, a feature that is useful for

| Audio Multiplexing<br />*Let users hear multiple audio sources at once that overlap.*              |
| -------------------------------------------------------------------------------------------------- |
...
## Using Audio Multiplexing[​](#using-audio-multiplexing "Direct link to Using Audio Multiplexing")

To get started using Audio Multiplexing, you first need to create a Publishing token with [Multisource](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md) and have multiple audio sources ready to test, each assigned a unique `sourceID` at the publisher.

Not familiar with our JavaScript SDK?

Audio Multiplexing is a complex feature made available through our [Client SDKs](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md).

Once you're streaming multiple audio sources, the next step is to set up the [Viewer](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/web/sdk.md#viewing-a-stream) so that the incoming audio sources can be correctly multiplexed. When connecting to the Viewer, there are a [number of parameters available in the SDK](https://millicast.github.io/millicast-sdk/View.html#connect) you can adjust depending on your workflow. Some parameters of note for audio multiplexing include:
...
### Are audio streams encrypted[​](#are-audio-streams-encrypted "Direct link to Are audio streams encrypted")
...
### Only one audio track is loading[​](#only-one-audio-track-is-loading "Direct link to Only one audio track is loading")
...
### Audio is dropping in and out[​](#audio-is-dropping-in-and-out "Direct link to Audio is dropping in and out")
