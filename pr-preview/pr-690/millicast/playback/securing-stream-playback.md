# Securing Stream Playback

[Securing the playback of your Broadcast](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md#create-a-publish-token) is one of the most important features provided by the OptiView Real-time platform. Although securing your stream is *optional*, it is **highly recommended** for broadcasts that are non-public or paywalled.

To view a secured stream you first need to create a Subscribe token. With your Subscribe token in hand, you can then use it to authenticate a connection and view the broadcast. This guide outlines [how to create a token](/documentation/pr-preview/pr-690/millicast/playback/securing-stream-playback.md#creating-a-subscribe-token-to-playback-a-secure-stream) and [how to view a secure stream](/documentation/pr-preview/pr-690/millicast/playback/securing-stream-playback.md#viewing-a-secured-stream).

## Creating a subscribe token to playback a secure stream[​](#creating-a-subscribe-token-to-playback-a-secure-stream "Direct link to Creating a subscribe token to playback a secure stream")

To secure broadcast playback, you need to enable the **Secure Viewer** setting when creating a Publishing token. For more information on enabling Secure Viewer, explore the [Creating a Publishing Token guide](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md#create-a-publish-token).
...
To create a Subscribe token, follow the[ Creating a Subscribe Token guide](/documentation/pr-preview/pr-690/millicast/subscribe-tokens.md), which outlines how to create a token in the OptiView Real-time Streaming Dashboard, or the [Token API](/documentation/pr-preview/pr-690/millicast/token-api.md) guide which outlines how to create tokens programmatically. Once created, your token will be a string of alphanumeric characters looking something like the following:

`fff04a5a1c02b2b8d48a9133e8461985aa482066cc3e9ed487baaac89588e26f`

With your token in hand, proceed to [Viewing a Secured Stream](/documentation/pr-preview/pr-690/millicast/playback/securing-stream-playback.md#viewing-a-secured-stream).

## Viewing a secured stream[​](#viewing-a-secured-stream "Direct link to Viewing a secured stream")

There are **two** main ways you can use the Subscribe token:

1. To view a stream via the [OptiView Hosted Viewer](/documentation/pr-preview/pr-690/millicast/playback/hosted-viewer.md).
2. To view a stream via your own streaming app.

### Viewing a secured stream with the hosted viewer[​](#viewing-a-secured-stream-with-the-hosted-viewer "Direct link to Viewing a secured stream with the hosted viewer")

[Playing back with the Hosted Viewer](/documentation/pr-preview/pr-690/millicast/playback/hosted-viewer.md) is a common way to test the OptiView Real-time platform. If you haven't already tested out the Hosted Viewer it is recommended you explore the [Hosted Viewer Playback guide](/documentation/pr-preview/pr-690/millicast/playback/hosted-viewer.md).
...
![](/documentation/pr-preview/pr-690/assets/images/image-6aa5f0fd58aed0a2ae4c77e77e6d7301.png)
...
![](/documentation/pr-preview/pr-690/assets/images/2d58054-image-fc4ff686960822911737d793a8d41b04.png)

If you attempt to join the stream without a valid Subscribe token, you'll receive a `401 Unauthorized` error:

<!-- -->

![](/documentation/pr-preview/pr-690/assets/images/25a57ae-image-d8750f44734ad32f199490590000a59a.png)

Subscribe token exposed in the URL

For both the Embedded Viewer and the Hosted Viewer the Subscribe token is exposed in the URL. Unless your are using [IP filtering](/documentation/pr-preview/pr-690/millicast/distribution/access-control/allowed-origins.md#ip-filters), users can share the URL and by extension the stream itself. To prevent this, each user should be served a Subscribe token unique to them. To learn about how to accomplish this continue to [Preventing Stream Sharing with Subscribe Tokens](/documentation/pr-preview/pr-690/millicast/playback/securing-stream-playback.md#preventing-stream-sharing-with-subscribe-tokens).

#### Viewing a secured stream with the embedded viewer[​](#viewing-a-secured-stream-with-the-embedded-viewer "Direct link to Viewing a secured stream with the embedded viewer")
...
### Preventing stream sharing with subscribe tokens[​](#preventing-stream-sharing-with-subscribe-tokens "Direct link to Preventing stream sharing with subscribe tokens")

Whilst using the Hosted Viewer is a great option, you may have noticed the Subscribe token is exposed in the URL. For a truly secure playback experience, viewers should be served a Subscribe token that is unique to them and can't be shared. This is accomplished with the [Binds IP on Usage](/documentation/pr-preview/pr-690/millicast/distribution/access-control/allowed-origins.md#bind-ip-on-usage) feature, which binds the Subscribe token to the IP address of the first person who joins the stream with that specific Subscribe token.

**For example:**

> Angelik may be served the stream URL<br />`https://viewer.millicast.com?streamId=DevAcc/WorldCup2023&token=fff04a5a1c02b2b8d48a9133e84619889588e26f`<br /><!-- -->which she uses to join the stream.
>
> Because this token has [Binds IP on Usage](/documentation/pr-preview/pr-690/millicast/distribution/access-control/allowed-origins.md#bind-ip-on-usage) enabled and set to bind to the first IP address, her IP address is saved by the OptiView Real-time CDN. If Angelik were to then share the URL to Jayson, he would be unable to join the stream since his IP address doesn't match Angelik's.
>
...
To learn more about other access control features you can enable for your Subscribe token, check out the [Access Control guide](/documentation/pr-preview/pr-690/millicast/distribution/access-control.md#subscribing-access-control).

### Viewing secure streams with your own viewer app[​](#viewing-secure-streams-with-your-own-viewer-app "Direct link to Viewing secure streams with your own viewer app")

Building with the OptiView Real-time SDKs for the first time?

Check out the [Getting Started](/documentation/pr-preview/pr-690/millicast.md) tutorial for a quick and easy introduction to using the OptiView Real-time SDKs and platform.

To view a secure stream in your own viewer app, you'll need to create a Subscribe token. Depending on the size of your platform, you may want to create these tokens programmatically using a token server and the [Token APIs](/documentation/pr-preview/pr-690/millicast/token-api.md). This part of the guide gives an example of how to programmatically create a Subscribe token, and how to use it to authenticate a connection to a viewer.

To get started we first need to define a function, `getToken` to create our Subscribe token. The [Create Token](/documentation/pr-preview/pr-690/millicast/api/subscribe-token-v-1-create-token.md) API provides a UI to help with defining our API call. Each Subscribe token that our `getToken` function creates will be associated with a single stream denoted by the `streamName`, expire after one hour (3600 seconds), and bind to the IP of the first person who connects to the stream with that token. In other words, this token will only grant access to a specific stream, for one hour, and for one user.
...
Using code from the [2. Create a Streaming Web App](/documentation/pr-preview/pr-690/millicast/getting-started/creating-real-time-streaming-web-app.md) tutorial, we can put everything together to create a very basic web app that uses a Subscribe token to view a secure stream.
...
When you run the web app, you'll see three text boxes to input an [account ID](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md), [Livestream Name](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md), and [API Secret key](/documentation/pr-preview/pr-690/millicast/about-dash.md#settings).

<!-- -->

![](/documentation/pr-preview/pr-690/assets/images/083b1aa-image-e3934e61edbb38d02ac1c635c8ff0ea1.png)
...
This error occurs if the Subscribe token, Account ID, or Stream Name is incorrect when attempting to join a stream. Check that you are correctly creating a [Subscribe token](/documentation/pr-preview/pr-690/millicast/subscribe-tokens.md), and using the correct [Stream Name](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md) and [Account ID](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md).

### CORS related errors[​](#cors-related-errors "Direct link to CORS related errors")
