...
To create a multi-view experience you must capture multiple video or audio feeds and then broadcast them as a [multi-source stream](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md). Once broadcasting a multi-source stream, you can view the stream using the OptiView viewer app, or by building your own multi-view application. OptiView Real-time Streaming also supports [Audio Multiplexing](/documentation/pr-preview/pr-690/millicast/playback/audio-multiplexing.md) for mixed audio playback.

## Multi-view with the OptiView viewer[​](#multi-view-with-the-optiview-viewer "Direct link to Multi-view with the OptiView viewer")

Once you have created a [Multisource stream](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md), you can open the stream viewer from the [OptiView Real-time Streaming Dashboard](https://streaming.dolby.io/#/tokens) or by navigating to:
...
![](/documentation/pr-preview/pr-690/assets/images/Select_multiview-d618957a6283fe804c251648207a2c2a.jpg)
...
## Creating a Multi-view web application[​](#creating-a-multi-view-web-application "Direct link to Creating a Multi-view web application")

OptiView Real-time Streaming supports [Multisource Playback](/documentation/pr-preview/pr-690/millicast/playback/source-and-layer-selection.md) via the [Client SDKs](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md), allowing you to build your own multi-view experience for your app or platform.

Before getting started building a multi-view application it is worth understanding;

1. How to broadcast [Multisource Streams](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md).
2. How to [Create a Basic Streaming Web App](/documentation/pr-preview/pr-690/millicast/getting-started/creating-real-time-streaming-web-app.md).
3. What [Broadcast Events](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/viewer-events.md) are and how to use them.
4. How the OptiView Real-time platform organizes and handles [Multisource Playback](/documentation/pr-preview/pr-690/millicast/playback/source-and-layer-selection.md).

### Store and track incoming Multisource feeds[​](#store-and-track-incoming-multisource-feeds "Direct link to Store and track incoming Multisource feeds")

Not building a Web App?

All OptiView Real-time [Client SDKs](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md) support building Multi-view applications. Although the below example is using JavaScript the principles are the same for each SDK.

The OptiView Real-time platform tracks broadcasts by their `account ID` and `stream name` and individual streams within broadcasts by their `sourceID`, a unique identifier that can be used for selecting feeds to render from the [viewer node](/documentation/pr-preview/pr-690/millicast/playback/source-and-layer-selection.md). Unlike a traditional broadcast where there is only one stream to playback, a multi-view application must account for multiple feeds arriving asynchronously. To accomplish this, the application should [listen for streams using a `broadcastEvent`](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/viewer-events.md#using-events), and store the stream `sourceID` as it becomes active.
...
### Add video elements and render feeds[​](#add-video-elements-and-render-feeds "Direct link to Add video elements and render feeds")

Once we've captured the `sourceID` of an incoming stream, we need to signal to the Viewer node which *track* the stream will play on. The OptiView Real-time SDKs include a function that allows you to [dynamically add a track to the Viewer node](/documentation/pr-preview/pr-690/millicast/playback/source-and-layer-selection.md#dynamic-viewer-track) called `addRemoteTrack`.

[addRemoteTrack](https://millicast.github.io/millicast-sdk/View.html#addRemoteTrack) requires the media type of the incoming stream (*audio or video*) and a [`MediaStream`](https://developer.mozilla.org/en-US/docs/Web/API/MediaStream), an interface that signals a stream of media content. `addRemoteTrack` will then return a promise that will be resolved when the [`RTCRtpTransceiver`](https://developer.mozilla.org/en-US/docs/Web/API/RTCRtpTransceiver) is assigned a `mid` value.

These newly created *Transceivers* can be stored alongside the `sourceID`, ready for when it is time to render the feed in the app.
...
At this stage, all the pieces are together, however, the stream won't yet render. This is because you have yet to tell the OptiView Viewer node which stream to project onto the `Transceiver`. This is done using the [`project` function](/documentation/pr-preview/pr-690/millicast/playback/source-and-layer-selection.md) which tells the node to begin *projecting* the stream, identified by its `sourceID`, onto the `Transceiver`.
...
### Final result[​](#final-result "Direct link to Final result")
...
[](https://dolby.io/wp-content/uploads/2022/11/multiview-vid.mp4)

## Assigning lower-quality layers to small tiles[​](#assigning-lower-quality-layers-to-small-tiles "Direct link to Assigning lower-quality layers to small tiles")
...
## Limitations of Multi-view[​](#limitations-of-multi-view "Direct link to Limitations of Multi-view")
