...
## Troubleshooting Client Playback Issues[​](#troubleshooting-client-playback-issues "Direct link to Troubleshooting Client Playback Issues")
...
For broadcast issues, the [Live Monitoring](/documentation/pr-preview/pr-690/millicast/live-monitoring.md) tool is available from your account [Streaming Dashboard](/documentation/pr-preview/pr-690/millicast/about-dash.md) or by using the [Broadcast Monitoring](/documentation/pr-preview/pr-690/millicast/api/monitoring-list-recent-streams.md) REST API to get more insight into what may cause problems with a stream.

### Diagnostics Report[​](#diagnostics-report "Direct link to Diagnostics Report")
...
To help facilitate investigating out why a client is experiencing a degraded experience, there is a utility function available from the [Client SDKs](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md) called `diagnose()`. This method gathers up pertinent details such as:
...
![](/documentation/pr-preview/pr-690/assets/images/feature-diagnostics-1f8f385e625cce87ab51b0ad3162492c.png)
...
### How-to Diagnose with THEOplayer[​](#how-to-diagnose-with-theoplayer "Direct link to How-to Diagnose with THEOplayer")

We have a more complete guide on this topic, refer to our [THEOplayer specific guide](/documentation/pr-preview/pr-690/millicast/web-player-diagnostics.md).

### How-to Diagnose with the Web SDK[​](#how-to-diagnose-with-the-web-sdk "Direct link to How-to Diagnose with the Web SDK")
...
* A large number of logging events will be buffered and included, but this number can be extended or shortened by using the **minLogLevel** and **historySize** to indicate which logs to include and how many of them. The default is `TRACE` but can be changed to *INFO* or *DEBUG* to increase or decrease the amount of detail respectively.
...
## Ongoing Monitoring of Client Analytics[​](#ongoing-monitoring-of-client-analytics "Direct link to Ongoing Monitoring of Client Analytics")

It is common practice to proactively monitor all clients for indicators of streaming issues. The [Client SDKs](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md) provide access to a `stats` event that emits a snapshot of WebRTC indicators at a frequent interval. This data can be used in combination with an **Application Performance Monitoring (APM)** solution or Observability tool to alert and integrate with other third-party analytics services for a complete picture of the end-viewer experience.

<!-- -->

![](/documentation/pr-preview/pr-690/assets/images/feature-diagnostics-1f8f385e625cce87ab51b0ad3162492c.png)

### Analytics Collectors[​](#analytics-collectors "Direct link to Analytics Collectors")
...
* [Datazoom](/documentation/pr-preview/pr-690/millicast/playback/client-analytics/datazoom.md)
* [Datadog](/documentation/pr-preview/pr-690/millicast/playback/client-analytics/datadog.md)

The same pattern can be used with other tools such as Grafana, Splunk, New Relic, etc.

Viewer Reporting Analytics API

The [Viewer Reporting](/documentation/pr-preview/pr-690/millicast/api/reporting/advanced-analytics-api.md) REST API is a premium [Analytics APIs](/documentation/pr-preview/pr-690/millicast/analytics-api.md) option made available to enterprise customers. It offers key insights from our server logs to help identify viewers, location, and other metrics. If you would like to use this endpoint, please [submit a support ticket](https://support.dolby.io/hc/en-au) or reach out to your [sales/solutions](https://optiview.dolby.com/contact/) contact.

### How-to Configure Stats Collection Frequency[​](#how-to-configure-stats-collection-frequency "Direct link to How-to Configure Stats Collection Frequency")
...
## Data Dictionary[​](#data-dictionary "Direct link to Data Dictionary")

The data measures and metrics gathered for diagnostics are described below.

### Software Identification[​](#software-identification "Direct link to Software Identification")
...
### Client Environment[​](#client-environment "Direct link to Client Environment")
...
### Client Connection[​](#client-connection "Direct link to Client Connection")
...
### Content Characteristics[​](#content-characteristics "Direct link to Content Characteristics")
...
### Playback Performance[​](#playback-performance "Direct link to Playback Performance")
...
| **jitter**                   | Deviation in packet reception times is called jitter and measured in milliseconds. The purpose of a jitter buffer is to recombine packets into frames for smooth playout.<br /><br />The **jitterBufferDelay** is the sum of time in seconds each frame takes from first packet received by the buffer to the time it is emitted and exits the buffer. The **jitterBufferEmittedCount** is the total number of frames.                                                                                        |                                                                                                                                                                                                                                                          |
| **packetRate**               | The throughput between client and server can be measured by the client as an estimated bandwidth from CDN to player for packets received per second.<br /><br />The **totalBytesReceived** and **totalPacketsReceived** is the sum of bytes and RTP packets respectively. **PacketsLost** is the total number of RTP packets sent by the server but not received by the client which can be used to calculate **packetsLossRatio** and **packetsLossDelta** which is packets lost since the last measurement. |                                                                                                                                                                                                                                                          |
