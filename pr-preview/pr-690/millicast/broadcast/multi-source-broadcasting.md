...
1. [Multi-view streams](/documentation/pr-preview/pr-690/millicast/playback/multiview.md): Multi-view is a feature whereby viewers ingest and render multiple streams simultaneously inside a browser or mobile native applications. Once rendered these streams can be switched between, offering the viewer the ability to control how they view and listen to the content. Multi-view is supported by all of our [Client SDKs](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md) for playback.
2. [Audio Multiplexing](/documentation/pr-preview/pr-690/millicast/playback/audio-multiplexing.md): Audio Multiplexing is a feature that allows viewers to receive multiple overlapping audio streams in a conference-like experience, where each audio stream is emphasized or deemphasized based on activity.
...
## Multi-source Workflow[​](#multi-source-workflow "Direct link to Multi-source Workflow")

Multi-source broadcasting is accomplished by enabling a token for multi-source and then publishing multiple quality layers or contributing sources using the same **publishing token** and **stream name**, but each with a unique **source id** and **simulcast id**. If you do not provide a source id, a second feed will overwrite the previous stream, which is common when trying to setup a [Redundant Ingest ](/documentation/pr-preview/pr-690/millicast/broadcast/redundant-ingest.md)in case of an encoder failure.

Enable Multi-source

To use Multi-source, a [publish Token](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md) must have [multi-source](/documentation/pr-preview/pr-690/millicast/managing-your-tokens.md#advanced-settings) enabled in the token settings. This can be done with the REST API or by editing the token from the dashboard.

Additionally, [Geo-cascading](/documentation/pr-preview/pr-690/millicast/distribution/multi-region-support/geo-cascading.md) is required when publishing to multiple regions for [Multi-view](/documentation/pr-preview/pr-690/millicast/playback/multiview.md) or [Redundant Ingest](/documentation/pr-preview/pr-690/millicast/broadcast/redundant-ingest.md).

When multi-source is enabled, you use the `sourceId` attribute when publishing a stream.

* If the `sourceId` is undefined, the stream is treated as the default or *main* stream to ensure backward compatibility.
...
* When there are multiple unique `sourceId` attributes the stream plays the **Main** source.
  <!-- -->
  * You can [enable multi-view streams](/documentation/pr-preview/pr-690/millicast/playback/multiview.md) on playback to view the available sources.
...
![](/documentation/pr-preview/pr-690/assets/images/multi-source-overview-a2291b5e8266ca26e7b1da5d5b2d33db.png)

### Multi-Bitrate Contribution and Simulcast[​](#multi-bitrate-contribution-and-simulcast "Direct link to Multi-Bitrate Contribution and Simulcast")
...
See the [Simulcast](/documentation/pr-preview/pr-690/millicast/distribution/using-webrtc-simulcast.md) guide for additional details on setting up these workflows.

### Publishing sources[​](#publishing-sources "Direct link to Publishing sources")

You can configure multiple publishing sources to enable simulcast or provide a multi-view experience of different content quickly using the [Multi-source Builder](/documentation/pr-preview/pr-690/millicast/multi-source-builder.md). You can also use the multi-source hosted player, which is a streaming video player, as a standalone web application or embed it into your website with an `<iframe>`.

### Multiple Points of View[​](#multiple-points-of-view "Direct link to Multiple Points of View")
...
See the [Multi-view](/documentation/pr-preview/pr-690/millicast/playback/multiview.md) guide for an explanation of that workflow for configuring playback.

### Broadcast URLs[​](#broadcast-urls "Direct link to Broadcast URLs")
...
* [Multi-source RTMP](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md#multi-source-rtmp)
* [Multi-source SRT](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md#multi-source-srt)

See [Publish Parameters](/documentation/pr-preview/pr-690/millicast/broadcast.md#publish-parameters) for descriptions of the supported broadcast parameters for each protocol.
...
Learn more about how to use the [Multi-source Builder](/documentation/pr-preview/pr-690/millicast/multi-source-builder.md).

#### Multi-source RTMP[​](#multi-source-rtmp "Direct link to Multi-source RTMP")
...
2. Gather the **RTMP publish path** and **RTMP publish stream name**. Depending on your choice of encoder, these may be configured as a single combined URL or as a separate URL and token. For more detailed instructions on locating these, see [How to find your RTMP publish URL](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md#how-to-find-your-rtmp-publish-url)

| RTMP publish path                            | Separator | RTMP publish stream name  |
| -------------------------------------------- | --------- | ------------------------- |
| rtmps\://rtmp-phx-1.millicast.com:443/v2/pub | /         | myStreamName?token=abc123 |

3. Configure your *main* source. Typically, this will be your highest-quality input in the case of MBR or or the source you want to be presented initially in multi-view. You will add the `sourceId` parameter and the `simulcastId` parameter. For the main source, the simulcastId will not have a value.
...
#### Multi-source SRT[​](#multi-source-srt "Direct link to Multi-source SRT")

Steps to build a broadcast URL suitable for SRT encoders:

1. Choose your broadcast cluster region. This should be the geographically closest location from which you will be broadcasting and cannot be "Auto".
2. Gather the **SRT publish path** and **SRT stream ID**. Depending on your choice of encoder, these may be configured as a single combined URL or as a separate URL and token. For more detailed instructions on locating these, see [Generate the SRT publish URL](/documentation/pr-preview/pr-690/millicast/broadcast/srt.md#3-generate-the-srt-publish-url).
...
* [Publishing with the SDK](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md#publishing-with-the-sdk)
* [Publishing with the hosted broadcaster in the dashboard](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md#publishing-with-the-hosted-broadcaster-in-the-dashboard)

### Publishing with the SDK[​](#publishing-with-the-sdk "Direct link to Publishing with the SDK")
...
For the native desktop client, the `sourceId` is set through a parameter for the publish function. For more information on setting the `sourceId`, see "Configure your publishing session" for [desktop](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/desktop/getting-started-with-publishing.md#4-configure-your-publishing-session), [Android](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/android/sdk/getting-started-with-publishing.md#44-configure-publishing-options), and [iOS](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/ios/sdk/getting-started-with-publishing.md#24-configure-publishing-options).

> Note: The `sourceId` should be a unique value and by default is undefined. Identical `sourceId` values should only be used for specifying [redundant streams](/documentation/pr-preview/pr-690/millicast/broadcast/redundant-ingest.md).
...
### Publishing with the Hosted Broadcaster in the Dashboard[​](#publishing-with-the-hosted-broadcaster-in-the-dashboard "Direct link to Publishing with the Hosted Broadcaster in the Dashboard")
...
In the *Live broadcast* section and click the `BROADCAST` button.
...
See the [How to Broadcast in Dashboard](/documentation/pr-preview/pr-690/millicast/how-to-broadcast-in-dashboard.md) guide to learn more details about the options available for configuring the publish token and working with this dashboard broadcast application.

## Playback[​](#playback "Direct link to Playback")

There are multiple ways to playback and consume a multi-view feed:

* Use the [Dolby-hosted player](/documentation/pr-preview/pr-690/millicast/how-to-broadcast-in-dashboard.md) (easiest for testing web)
* Build your own [multi-view app for Web](/documentation/pr-preview/pr-690/millicast/getting-started/creating-real-time-streaming-web-app.md)
* Build your own [multi-view app with the SDK](/documentation/pr-preview/pr-690/millicast/playback/players-sdks.md)

## Troubleshooting[​](#troubleshooting "Direct link to Troubleshooting")

### Broadcasting best practices[​](#broadcasting-best-practices "Direct link to Broadcasting best practices")

* There is a 12Mbps total connection limit on the playback side. If you are using high-bitrate source feeds, we recommend sending at least a high-quality and a low-quality version of each source so that in a multi-view playback, the large image can be high-bitrate and the small thumbnail tiles can be set to the low-bitrate. For more information, see [Limitations of Multi-view](/documentation/pr-preview/pr-690/millicast/playback/multiview.md#limitations-of-multi-view). This is primarily for broadcasting with RTMP or SRT. If you are using WebRTC to publish and your publishing client supports Simulcast (such as our hosted broadcaster), this is handled automatically.
...
### Playback best practices[​](#playback-best-practices "Direct link to Playback best practices")
...
* The `priority` parameter lets you set the priority of each feed. For redundant streams the primary feed should be assigned a consistent value for all input feeds, regardless of sourceId or simulcastId, and backups should mirror the primary's sourceID and simulcastId with a lower priority. In the case of any problem with the primary feed, viewers receive the available feed that has the highest priority. Not setting any priorities or setting the same priorities results in receiving the most recently published feed. For more information, see [Redundant Ingest](/documentation/pr-preview/pr-690/millicast/broadcast/redundant-ingest.md).
