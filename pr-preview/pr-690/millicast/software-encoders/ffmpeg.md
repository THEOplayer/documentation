...
Broadcasts are started using the `ffmpeg` command-line to forward a source using either [RTMP](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md) or [SRT](/documentation/pr-preview/pr-690/millicast/broadcast/srt.md) broadcast contribution protocols which are then streamed as [WebRTC](/documentation/pr-preview/pr-690/millicast/broadcast/webrtc-and-whip.md) for global real-time distribution.
...
| ---------------- | ------------------------------------------------ |
| -stream\_loop -1 | Loop the video indefinitely                      |
| -vb 3000k        | Video Bitrate setting of 3,000 Kbps              |
| -acodec aac      | AAC audio codec                                  |
| -ac 2            | Audio channels for stereo                        |
| -ab 96000        | Audio Bitrate setting of 96 Kbps                 |
| -ar 48000        | Audio Sample Rate of 48 Kbps                     |
...
In order to broadcast with RTMP, you will need to have your **RTMP publish path** and **RTMP publish stream name** available. See the [RTMP Broadcast Guide](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md#how-to-find-your-rtmp-publish-url) for details on how to retrieve these values.
...
### How-to Publish an H.264 RTMP Video Stream[​](#how-to-publish-an-h264-rtmp-video-stream "Direct link to How-to Publish an H.264 RTMP Video Stream")
...
### How-to Publish an H.265 RTMP Video Stream[​](#how-to-publish-an-h265-rtmp-video-stream "Direct link to How-to Publish an H.265 RTMP Video Stream")

The Millicast platform supports HEVC (H265) broadcast pass-through, but playback of this codec is not widely supported across all web browsers and devices. To view an HEVC stream, you should use Safari 17.2+ or the [iOS](/documentation/pr-preview/pr-690/millicast/playback/players-sdks/ios/sdk.md) Client SDK which provide decoding support.
...
### How-to Publish an AV1 RTMP Video Stream[​](#how-to-publish-an-av1-rtmp-video-stream "Direct link to How-to Publish an AV1 RTMP Video Stream")
...
### How-to Publish an RTSP Video Stream[​](#how-to-publish-an-rtsp-video-stream "Direct link to How-to Publish an RTSP Video Stream")
...
### How-to Simulcast a Multi-Source MBR Video Stream[​](#how-to-simulcast-a-multi-source-mbr-video-stream "Direct link to How-to Simulcast a Multi-Source MBR Video Stream")

This example demonstrates sending the same video with multiple contribution layers. The `&sourceId` [publishing parameter](/documentation/pr-preview/pr-690/millicast/broadcast/publishing-parameters.md) is used to distinguish each source while using `&videoOnly` so the audio is only sent with the main feed. Also see the [Multi-Source Broadcasting](/documentation/pr-preview/pr-690/millicast/broadcast/multi-source-broadcasting.md) for more about Multi-bitrate contribution.
...
In order to broadcast with SRT, you will need to have your **SRT publish path** and **SRT stream ID** available. See the [SRT Broadcast Guide](/documentation/pr-preview/pr-690/millicast/broadcast/srt.md) for details on how to retrieve these values.
...
### How-to Check if SRT is Supported in Your Installation[​](#how-to-check-if-srt-is-supported-in-your-installation "Direct link to How-to Check if SRT is Supported in Your Installation")
...
### How-to Simulcast a Redundant Ingest SRT Video Stream[​](#how-to-simulcast-a-redundant-ingest-srt-video-stream "Direct link to How-to Simulcast a Redundant Ingest SRT Video Stream")

This example demonstrates \[redundant ingest]/millicast/broadcast/redundant-ingest/index.mdx) where a second publishing source is used to recover from a failed broadcast source. The `&priority=100` [publishing parameter](/documentation/pr-preview/pr-690/millicast/broadcast/publishing-parameters.md) is used to indicate which is the primary and which is the backup feed. **The SRT URL must be URLencoded.**
...
### Stuttering video[​](#stuttering-video "Direct link to Stuttering video")
...
This guide covered broadcasting with `ffmpeg`. To test and view the stream you can use the dashboard or any of the other [playback](/documentation/pr-preview/pr-690/millicast/playback.md) methods.

Continue exploring other [software encoders](/documentation/pr-preview/pr-690/millicast/software-encoders.md) and solutions you can use for [broadcasting](/documentation/pr-preview/pr-690/millicast/broadcast.md) your real-time stream.
