# Software Encoders

A software encoder can take raw video frames and convert it into a digital format that is compatible for distribution with OptiView Real-time Streaming. This type of integration can typically be done without additional hardware requirements and eases adoption of real-time streaming workflows.

## Integration Guides[​](#integration-guides "Direct link to Integration Guides")

Review these guides for how to setup your preferred tools for real-time streaming integrations.

<!-- -->

[![FFMPEG](https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/FFmpeg_Logo_new.svg/2560px-FFmpeg_Logo_new.svg.png)](/documentation/pr-preview/pr-690/millicast/software-encoders/ffmpeg.md)

[![OBS](https://upload.wikimedia.org/wikipedia/commons/d/d3/OBS_Studio_Logo.svg)](/documentation/pr-preview/pr-690/millicast/software-encoders/obs.md)

[![GStreamer](https://upload.wikimedia.org/wikipedia/commons/thumb/d/db/Gstreamer-logo.svg/2560px-Gstreamer-logo.svg.png)](/documentation/pr-preview/pr-690/millicast/software-encoders/gstreamer.md)

[![vMix](https://www.vmix.com/images/2017/logos/web/vmix-logo/vMix-Logo-Black.png)](/documentation/pr-preview/pr-690/millicast/software-encoders/vmix.md)

[![Flowcaster](data:image/png;base64,)](/documentation/pr-preview/pr-690/millicast/software-encoders/flowcaster.md)

[![LiveU](https://cdn-liveutv.pressidium.com/wp-content/uploads/2021/06/LiveU_Logo_On_Whtite.png)](/documentation/pr-preview/pr-690/millicast/software-encoders/liveu-studio.md)

### Drastic Technologies[​](#drastic-technologies "Direct link to Drastic Technologies")

**FlowCaster** from Drastic Technologies lets you share high quality video over IP for remote collaboration and review workflows. This tool supports broadcasting with [WHIP](/documentation/pr-preview/pr-690/millicast/broadcast/webrtc-and-whip.md).

[How-to integrate with Flowcaster](/documentation/pr-preview/pr-690/millicast/software-encoders/flowcaster.md)

### FFmpeg[​](#ffmpeg "Direct link to FFmpeg")

**FFmpeg** is a free open-source software project with command-line tools for handling video, audio, and other multimedia. The `ffmpeg` command-line tool can be used to forward streams with [RTMP](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md). This can be helpful for quickly streaming a media file from disk.

[How-to integrate with FFmpeg](/documentation/pr-preview/pr-690/millicast/software-encoders/ffmpeg.md)

### GStreamer[​](#gstreamer "Direct link to GStreamer")

**GStreamer** is a free open-source software project and multimedia framework to build media processing pipelines. The tools can be used to ingest streams with [WHIP](/documentation/pr-preview/pr-690/millicast/broadcast/webrtc-and-whip.md). This can be helpful for complex workflows that operate on media streams.

[How-to integrate with GStreamer](/documentation/pr-preview/pr-690/millicast/software-encoders/gstreamer.md)

### Open Broadcaster Software (OBS)[​](#open-broadcaster-software-obs "Direct link to Open Broadcaster Software (OBS)")

**OBS** is a free open-source application for broadcasting and recording streams from your desktop computer. It supports broadcasting with [WebRTC](/documentation/pr-preview/pr-690/millicast/broadcast/webrtc-and-whip.md), [SRT](/documentation/pr-preview/pr-690/millicast/broadcast/srt.md), and [RTMP](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md) to support many different use cases.

[How-to integrate with OBS](/documentation/pr-preview/pr-690/millicast/software-encoders/obs.md)

### LiveU[​](#liveu "Direct link to LiveU")

**LiveU Studio** is a cloud production tool for live video feeds. It supports [WHIP](/documentation/pr-preview/pr-690/millicast/broadcast/webrtc-and-whip.md).

[How-to integrate with LiveU Studio](/documentation/pr-preview/pr-690/millicast/software-encoders/liveu-studio.md)

### vMix[​](#vmix "Direct link to vMix")

**vMix** is a Windows desktop applicationi that provides a vision mixer and encoder. You can use this tool as both a [RTMP](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md) or [NDI](/documentation/pr-preview/pr-690/millicast/broadcast/ndi.md) source.

[How-to integrate with vMix](/documentation/pr-preview/pr-690/millicast/software-encoders/vmix.md)

### Zoom[​](#zoom "Direct link to Zoom")

**Zoom** is a video collaboration platform. The application supports streaming use cases with [RTMP](/documentation/pr-preview/pr-690/millicast/broadcast/rtmp-and-rtmps.md) which allows you to distribute your meeting with OptiView Real-time Streaming.

[How-to integrate with Zoom Meetings](/documentation/pr-preview/pr-690/millicast/software-encoders/zoom.md)

## Learn more[​](#learn-more "Direct link to Learn more")

You can find examples of using [OBS](https://optiview.dolby.com/resources/blog/streaming/using-webrtc-in-obs-for-remote-live-production/), and more on the [developer blog](https://optiview.dolby.com/resources/blog/tag/encoder/).

## [![](/documentation/pr-preview/pr-690/img/github.svg)![](/documentation/pr-preview/pr-690/img/github_dark.svg)Explore GitHub samples](https://github.com/orgs/dolbyio-samples/repositories?q=encoder)

## [🔗Find related blog posts](https://optiview.dolby.com/resources/blog/tag/encoder/)
