...
To access the analytics APIs you must have an API token. To learn more on how to get your API token, please read the following article [Acquiring Your API Token](/documentation/pr-preview/pr-690/millicast/token-api.md).
...
## Access restrictions[​](#access-restrictions "Direct link to Access restrictions")
...
## Basic call structure[​](#basic-call-structure "Direct link to Basic call structure")
...
## Example tutorial: JS / Nodejs[​](#example-tutorial-js--nodejs "Direct link to Example tutorial: JS / Nodejs")
...
![](/documentation/pr-preview/pr-690/assets/images/analytics-accnt-call-btn-8abd2bda110e98e1ed5f019636d9c7fa.png)
...
![](/documentation/pr-preview/pr-690/assets/images/analytics-goog-graph1-2666258ba0e85961cf19c5384b686b23.png)
...
See more API calls here: [Account Analytics](/documentation/pr-preview/pr-690/millicast/api/analytics-account-total.md)

## Bandwidth per stream[​](#bandwidth-per-stream "Direct link to Bandwidth per stream")

The Analytics API allows you to not only get the bandwidth you need from your account itself but also from each individual stream. These calls follow the same rules as the call structure explained above ([Basic Call Structure](/documentation/pr-preview/pr-690/millicast/analytics-api.md#basic-call-structure)).

There is one slight difference we have to be aware of. For the fact that we are requesting data for a stream, in the query it is necessary to include the stream names along with the start and stop times. The use of multiple stream names is also supported. In order to use this feature you simply add each stream to a "streamNames" variable in the query, as an example your query stream might look like this:<br /><!-- -->startDate=2020-01-01\&stopDate=2020-04-01&**streamNames**=mystream&**streamNames**=otherstream<br /><!-- -->You can add up to 10 stream names at a time.
...
![](/documentation/pr-preview/pr-690/assets/images/analytics-goog-graph2-2299c39aafde9e04d6386c99f3ec13f7.png)
...
## Viewers per stream[​](#viewers-per-stream "Direct link to Viewers per stream")

Calculating the viewers per stream (daily or hourly), which region they viewed from, and how much bandwidth they consumed is a straightforward process. Navigate to the OptiView API reference and select the [Analytics Streams Geo Series API](/documentation/pr-preview/pr-690/millicast/api/analytics-streams-geo-series.md). Add your startDate, stopDate, resolution, and streamName. Additionally, in the top right corner, add your API Secret key found in the [Settings Tab](/documentation/pr-preview/pr-690/millicast/about-dash.md#settings) of the dashboard. Once all the fields have correct values click the `Try It!` button to get your data.

> 🚧 Get your data before it expires!
>
> Hourly data is only stored seven days after the stream started. For more information see [Access Restrictions](/documentation/pr-preview/pr-690/millicast/analytics-api.md#access-restrictions).

![](/documentation/pr-preview/pr-690/assets/images/millicastViewerCount-c6493468ab4dbb57492948385438b458.png)

## Troubleshooting[​](#troubleshooting "Direct link to Troubleshooting")

### No Results Returned for Stream[​](#no-results-returned-for-stream "Direct link to No Results Returned for Stream")

#### Special Characters in Stream Names[​](#special-characters-in-stream-names "Direct link to Special Characters in Stream Names")

When querying usage with the [Analytics](/documentation/pr-preview/pr-690/millicast/api/analytics-streams-series.md) APIs using `streamNames` it is expected that the streams have not used special characters such as `/`, `*`, or `--` as this will fail to report results. Recommendation is to avoid using special characters in stream names and sticking with alphanumeric characters and single dashes.
