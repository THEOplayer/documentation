...
When your signaling service is created, the default layout should be defined. Look at our [API references](/documentation/pr-preview/pr-690/ads/api/signaling/create-monetized-stream.md) for more information.

The rest of this page will go over how to override this initial chosen layout in the [signaling service](#override-layout-through-the-signaling-service) itself, or override it on the [player side](#override-the-layout-through-the-player).

## Override layout through the signaling service[​](#override-layout-through-the-signaling-service "Direct link to Override layout through the signaling service")

There are two ways to update the layout in the signaling service.

### Setting a new default layout[​](#setting-a-new-default-layout "Direct link to Setting a new default layout")

It is possible to update the default layout value on the signaling service. This can be done by making an [UpdateMonetizedStreamLayout](/documentation/pr-preview/pr-690/ads/api/signaling/update-monetized-stream-layout.md) API request to the signaling service with the new layout. The updated layout value will be the new default for all ad breaks.

### Signaling an ad break through the API[​](#signaling-an-ad-break-through-the-api "Direct link to Signaling an ad break through the API")
...
For more information on the scheduling API refer to the following section: [Scheduling breaks through the signaling service](/documentation/pr-preview/pr-690/ads/how-to-guides/scheduling-breaks.md#using-the-optiview-ads-service-rest-api).

## Override the layout through the player[​](#override-the-layout-through-the-player "Direct link to Override the layout through the player")
