...
![OptiView Ads architecture](/documentation/pr-preview/pr-690/ads/img/how_ads_works-light.svg)![OptiView Ads architecture](/documentation/pr-preview/pr-690/ads/img/how_ads_works-dark.svg)

## Infrastructure integration[​](#infrastructure-integration "Direct link to Infrastructure integration")
...
## Monetized streams[​](#monetized-streams "Direct link to Monetized streams")
...
* `layout`: Default experience layout, see [ad experience layout](/documentation/pr-preview/pr-690/ads/how-to-guides/override-layout.md).
...
### Creating a new monetized stream[​](#creating-a-new-monetized-stream "Direct link to Creating a new monetized stream")
...
### Updating an existing monetized stream[​](#updating-an-existing-monetized-stream "Direct link to Updating an existing monetized stream")
...
### Deleting an existing monetized stream[​](#deleting-an-existing-monetized-stream "Direct link to Deleting an existing monetized stream")
...
### Retrieving all monetized streams[​](#retrieving-all-monetized-streams "Direct link to Retrieving all monetized streams")
...
### Retrieving an individual monetized stream[​](#retrieving-an-individual-monetized-stream "Direct link to Retrieving an individual monetized stream")
...
Please refer to the [API reference](/documentation/pr-preview/pr-690/ads/api/signaling/theoads-api.md) for even more detailed information on the REST API.

### Player source[​](#player-source "Direct link to Player source")
...
In this URI, the `<optiview-ads-host>` points to the network endpoint where the OptiView Ads service is deployed, preferably a DNS entry pointing to the service IP endpoint and reachable from the CDN. Secondly, the `<monetized-stream-id>` corresponds to the `streamId` for the monetized stream. Finally, the `MANIFEST-URI` part points to the origin's manifest relative to the configured `origin` property of the monetized stream. The signaling service will concatenate the `origin` property and the MANIFEST-URI to build the origin manifest URI. For example:
...
Segment URLs in the media playlists should be absolute URLs however. Segment requests don't need to pass through the Signaling Service but should be fetched directly from the CDN to the origin so as to keep the benefit of scaling via the CDN. The `segmentOrigin` parameter should contain this publicly available endpoint to fetch the segments directly. The signaling service will concatenate the `segmentOrigin` parameter with the segment URIs in the playlists to build an absolute segment URI.

## Scheduling ad breaks[​](#scheduling-ad-breaks "Direct link to Scheduling ad breaks")

Once all the required monetized streams are configured, the next step is scheduling ad breaks for these monetized streams. To accurately schedule ad breaks, the origin manifest must be valid and include date and time indications. For HLS, this means the `EXT-X-PROGRAM-DATE-TIME` tag must be present. Ad breaks can be signaled through either the provided REST API or by including the relevant information in the manifest itself.
...
For scheduling ad breaks through the REST API, please refer to the [API definitions](/documentation/pr-preview/pr-690/ads/api/signaling/create-monetized-stream-break.md) and the example below.
...
For more in depth information on scheduling ad breaks we refer to our [How-to guide: Scheduling breaks](/documentation/pr-preview/pr-690/ads/how-to-guides/scheduling-breaks.md)

For more information on this topic we refer to our [workflow integration](/documentation/pr-preview/pr-690/ads/how-to-guides/workflow-integration.md).
