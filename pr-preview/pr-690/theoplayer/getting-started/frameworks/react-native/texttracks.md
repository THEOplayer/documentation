...
THEOplayer's `TextTrack` api gives developers the capability to manage and manipulate various types of text-based content. Some of its key functionalities include text track selection, styling, and listening to track and cue events.

## Types of text tracks[​](#types-of-text-tracks "Direct link to Types of text tracks")
...
## Side-loaded text tracks[​](#side-loaded-text-tracks "Direct link to Side-loaded text tracks")

In contrast to in-band and out-band text tracks, which are delivered to the player through the media content itself, the side-loaded text tracks can be configured by the user when setting a source to the player:
...
### iOS/tvOS[​](#iostvos "Direct link to iOS/tvOS")

On iOS and tvOS, support for side-loaded tracks is provided by a separate connector, which is enabled by adding the feature to the `react-native-theoplayer.json` (or `theoplayer-config.json`) file in your app folder.
...
## Listening to text track events[​](#listening-to-text-track-events "Direct link to Listening to text track events")

Out-band and side-loaded text tracks become accessible via the text track API once the player has loaded the media source. This event is signaled by the `PlayerEventType.LOADED_METADATA` event, which is dispatched by the [THEOplayer API](https://github.com/THEOplayer/react-native-theoplayer/blob/-/src/api/player/THEOplayer.ts):
...
Alternatively, the `PlayerEventType.TEXT_TRACK_LIST` event can be used to dynamically listen to text tracks that are being added, removed or changed. In-band text tracks, in particular, are created when their first cue is loaded, which can occur at any point during playback.
...
Similarly for text track cues, the `PlayerEventType.TEXT_TRACK` event provides knowledge on cues being added or removed. The cue contains a reference to the track it belongs too.
...
## Enabling a text track[​](#enabling-a-text-track "Direct link to Enabling a text track")

A subtitle or closed caption can be enabled by setting the player's `selectedTextTrack` property to the `uid` of the track, or `undefined` to select none.
...
Note that [THEOplayer React-Native UI](https://github.com/THEOplayer/react-native-theoplayer-ui) package provides a visual representation of the list of available text tracks, as well as the ability to enable/disable them.
...
Preview thumbnails is a feature that allows users to see a visual representation of different points or scenes within a video, making it easier to navigate and locate specific content of interest.

The thumbnails are typically contained in a dedicated thumbnail track, which is a text track of kind `metadata` with label `thumbnails`. The track can be either be side-loaded to the stream source, or contained in a stream manifest, as the demo sources in the [example application](/documentation/pr-preview/pr-690/theoplayer/getting-started/frameworks/react-native/example-app.md) demonstrate:
...
The [`@theoplayer/react-native-ui`](https://github.com/THEOplayer/react-native-theoplayer-ui) package provides a thumbnail viewer the interprets thumbnail cues and displays them above the progress bar.

![thumbnails](/documentation/pr-preview/pr-690/assets/images/thumbnails-c17c05ef4018a5a2ad04327f820ed6be.png)

## Styling subtitles and closed captions[​](#styling-subtitles-and-closed-captions "Direct link to Styling subtitles and closed captions")

In this section, we showcase the functionality of THEOplayer's `TextTrackStyle` API, which enables the customization of subtitles and closed captions' appearance. We will illustrate the practical implementation of this API through some examples.

### Style properties[​](#style-properties "Direct link to Style properties")
...
|                                                                                                                              |                                                                                                                                                    |
| ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![background](/documentation/pr-preview/pr-690/assets/images/texttrackstyle_background-c95e54ee5b2a2bc17a69cc40d280344f.png) | ```typescript
player.textTrackStyle.backgroundColor = 'red';
player.textTrackStyle.marginLeft = 45;

```                                               |
| ![size](/documentation/pr-preview/pr-690/assets/images/texttrackstyle_size-cba477b68686174a985054911bb4b474.png)             | ```typescript
player.textTrackStyle.windowColor = 'black';
player.textTrackStyle.fontColor = '#ffff00ff';
player.textTrackStyle.fontSize = '250%';

``` |
| ![size](/documentation/pr-preview/pr-690/assets/images/texttrackstyle_family-4ec69c505c8ee26bc7fa89802503ca25.png)           | ```typescript
