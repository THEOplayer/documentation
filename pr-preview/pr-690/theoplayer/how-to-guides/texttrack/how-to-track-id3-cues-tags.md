...
[ID3](https://en.wikipedia.org/wiki/ID3) is a type of metadata which can be inserted in HTTP livestreams. Once an ID3 cue is inserted, it'll be added to a THEOplayer [TextTrack](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/web/interfaces/TextTrack.html). The demo at <https://demo.theoplayer.com/audio-id3-metadata> demonstrates a usage of ID3 metadata. Just before the song changes, an `enter` event will be dispatched, the song information (title, album, etc.) is contained within this `enter` event, and can be used to update the UI.
...
## Tracking ID3[​](#tracking-id3 "Direct link to Tracking ID3")
...
### Listening for timed metadata events[​](#listening-for-timed-metadata-events "Direct link to Listening for timed metadata events")
...
### Track the enter event[​](#track-the-enter-event "Direct link to Track the enter event")

The snippet below explains how you correctly track the `enter` event, which is part of the [TextTrack API](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/web/interfaces/TextTrack.html). The `enter` event maps to the moment in time when the ID3 cue is relevant.

> ⚠ **Migrating from version 3.x or lower**
>
> Previously, the `exit` event was used to signal when an ID3 cue becomes relevant. As of THEOplayer version 4.0, the `enter` event is used instead.
...
* Demo: <https://demo.theoplayer.com/audio-id3-metadata>

## Resources[​](#resources "Direct link to Resources")

* [Reference API - TextTrack](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/web/interfaces/TextTrack.html)
