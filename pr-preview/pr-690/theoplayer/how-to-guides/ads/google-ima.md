...
Scheduling and tracking ads through the Google IMA ad integration is similar to the default ad integration, so you're advised to read through ["How to set-up VAST and VMAP](/documentation/pr-preview/pr-690/theoplayer/how-to-guides/ads/how-to-set-up-vast-and-vmap.md).
...
### Include the IMA SDK[​](#include-the-ima-sdk "Direct link to Include the IMA SDK")
...
### Specify the integration[​](#specify-the-integration "Direct link to Specify the integration")
...
### Add the `integration-ads-ima` dependency[​](#add-the-integration-ads-ima-dependency "Direct link to add-the-integration-ads-ima-dependency")
...
### Use the `GoogleImaIntegrationFactory`[​](#use-the-googleimaintegrationfactory "Direct link to use-the-googleimaintegrationfactory")
...
### Use a `GoogleImaAdDescription`[​](#use-a-googleimaaddescription "Direct link to use-a-googleimaaddescription")

Use a [`GoogleImaAdDescription`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/android/com/theoplayer/android/api/source/addescription/GoogleImaAdDescription.html) to schedule advertisements, as demonstrated below:
...
The Google IMA integration exposes events through the Ads API. More information is available at ["How to subscribe to ad events"](/documentation/pr-preview/pr-690/theoplayer/how-to-guides/ads/how-to-subscribe-to-ad-events.md#android-sdk).

The integration exposes a number of additional methods. These are available directly on the [`GoogleImaIntegration`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/android/com/theoplayer/android/api/ads/ima/GoogleImaIntegration.html) object, or indirectly through `player.ads.ima` (only for Kotlin). For example, [`schedule()`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/android/com/theoplayer/android/api/ads/ima/GoogleImaIntegration.html#schedule\(GoogleImaAdDescription\)) can be used to schedule ads dynamically, and [`requestAds()`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/android/com/theoplayer/android/api/ads/ima/GoogleImaIntegration.html#requestAds\(AdsRequest,AdsRenderingSettings\)) can be used to request ads through the native Google IMA API.

### Updating the Google IMA SDK[​](#updating-the-google-ima-sdk "Direct link to Updating the Google IMA SDK")

The IMA integration depends on Google's [IMA SDK for Android](https://developers.google.com/interactive-media-ads/docs/sdks/android/client-side/) from [Maven](https://mvnrepository.com/artifact/com.google.ads.interactivemedia.v3/interactivemedia). By adding the THEOplayer IMA integration to your Gradle dependencies, the Google IMA SDK is also automatically added to your app.

Our integration is tested with a specific version of the Google IMA SDK, and by default Gradle will automatically add the same version to your app. However, you can overwrite this [with a later version](https://developers.google.com/interactive-media-ads/docs/sdks/android/client-side/history) (at your own risk) by manually adding a dependency on `com.google.ads.interactivemedia.v3:interactivemedia` to your `build.gradle`.
...
You will also need the THEOplayer core SDK since the THEOplayer GoogleIMA integration extends its functionality.<br />To import the THEOplayer core SDK framework add:
