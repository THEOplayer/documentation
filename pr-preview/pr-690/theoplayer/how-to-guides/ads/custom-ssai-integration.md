...
### Integration ID[​](#integration-id "Direct link to Integration ID")
...
### ServerSideAdIntegrationFactory[​](#serversideadintegrationfactory "Direct link to ServerSideAdIntegrationFactory")
...
The `ServerSideAdIntegrationFactory` requires an implementation of the `ServerSideAdIntegrationHandler` interface. The handler will provide few callbacks that can be overridden which allows you to setup and clear the integration as needed.
...
When a new source is loaded into the player the `setSource(source: SourceDescription)` callback is triggered. It allows the integration to transform the source description, e.g. by calling an external service to replace the content URL (`TypedSource.src`), or by adding a fixed pre-roll linear ad to the list of ads (`SourceDescription.ads`).

#### Skipping an Ad[​](#skipping-an-ad "Direct link to Skipping an Ad")

When an ad is requested to be skipped, the `skipAd(ad: Ad)` is called. At this point the controller should call its own `skipAd()` method.

#### Resetting a Source[​](#resetting-a-source "Direct link to Resetting a Source")
...
#### Destroying the player[​](#destroying-the-player "Direct link to Destroying the player")

When the player is destroyed, the `destroy()` is called. This allows the integration to clean up any resources, such as UI views or event listeners.

#### Creating Ad objects[​](#creating-ad-objects "Direct link to Creating Ad objects")

When the advertisement provider reports an ad, an `AdBreak` and an `Ad` object should be created using `createAdBreak(props: AdBreakInit)` and `createAd(props: AdInit, adBreak: AdBreak?)` methods respectively from the controller. For example, for a preroll `AdBreak` containing 2 advertisements it would be:
...
#### Playing an Ad[​](#playing-an-ad "Direct link to Playing an Ad")
...
### CustomSsaiDescriptionSerializer[​](#customssaidescriptionserializer "Direct link to CustomSsaiDescriptionSerializer")

To hold the configuration parameters specific to your integration, you can create a new class that inherits from `CustomSsaiDescription`. This allows it to be set on a `TypedSource` through [`TypedSource.Builder.ssai()`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/android/com/theoplayer/android/api/source/TypedSource.Builder.html#ssai\(com.theoplayer.android.api.source.ssai.SsaiDescription\)). Then, register a `CustomSsaiDescriptionSerializer` using the integration ID and provide a way to serialize/deserialize the new `CustomSsaiDescription`.
