...
The THEOplayer iOS SDK provides the capability to pause an ongoing task. Doing so will store the file state of the partially downloaded stream to disk, and allow the task to be resumed even after the application runtime has been terminated. Note: See [CachingTask](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/ios/Protocols/CachingTask.html) for the `pause()` method.
