...
## Intercepting errors[​](#intercepting-errors "Direct link to Intercepting errors")
...
### Event Listener[​](#event-listener "Direct link to Event Listener")

Developers can subscribe to events and attach a callback function. This callback function implements the error handling.

This section describes how to use event handlers for the "top-level", "generic", "player" `error` event. This is the [`error`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/web/interfaces/PlayerEventMap.html#error) event which is dispatched through the `Player` interface. This event is fatal and prevents playback. (There are also `error` events, for example the [`error`](/documentation/pr-preview/pr-690/theoplayer/v11/api-reference/web/interfaces/ChromecastEventMap.html#error) event which is dispatched through the `Chromecast` interface. You handle these `error` events in the same fashion.)
...
Developers can query an occurred "generic player error" through the `errorObject` property of its related interface. For example, if the `error` event of the top-level `Player` interface would be dispatched for the Web SDK, then you can access the `errorObject` property of this interface.
...
"[Which error related events does the player expose](/documentation/pr-preview/pr-690/theoplayer/faq/which-error-related-events-does-player-expose.md)" lists which types of errors there are. Note that generally speaking only the "generic error event" is fatal. Other events, like the "Chromecast error event" and the "ad error event", are considered non-fatal because some type of playback can style continue.

"[Error codes](/documentation/pr-preview/pr-690/theoplayer/how-to-guides/miscellaneous/error/error-codes.md)" lists which error codes are available for the "generic error event".
...
* You could render your own error messages inside the video player.
  <!-- -->
  * The article at <http://demo.theoplayer.com/using-custom-error-messages> offers some help on this for the Web SDK.
