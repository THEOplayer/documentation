...
## Architecture: from flat channels to a component model[​](#architecture-from-flat-channels-to-a-component-model "Direct link to Architecture: from flat channels to a component model")
...
This means that creating a channel in V2 requires multiple API calls: one to create the channel, then one each for the ingest, engine, and distribution. See the [full example](/documentation/pr-preview/pr-690/theolive/api/full-example.md) for a complete walkthrough.

### V1 create channel → V2 equivalent[​](#v1-create-channel--v2-equivalent "Direct link to V1 create channel → V2 equivalent")
...
## Channel aliases → distributions[​](#channel-aliases--distributions "Direct link to Channel aliases → distributions")
...
| `fallback.enabled` / `fallback.src` | Use [redundancy](/documentation/pr-preview/pr-690/theolive/distribution/redundancy.md) (multiple engines on one distribution) |
| *(not available)*                   | `outputs.hesp`, `outputs.hlsMpegTs`, `maxBitrate`, `overrides`, `webRtc`, `dvr`, `security.refererBlocking`                   |

### Geo-blocking mode values[​](#geo-blocking-mode-values "Direct link to Geo-blocking mode values")
...
## Stream configuration → ABR ladders[​](#stream-configuration--abr-ladders "Direct link to Stream configuration → ABR ladders")
...
## Ingest configuration[​](#ingest-configuration "Direct link to Ingest configuration")
...
### Geo-blocking and IP blocking[​](#geo-blocking-and-ip-blocking "Direct link to Geo-blocking and IP blocking")
...
### Token security → security keys[​](#token-security--security-keys "Direct link to Token security → security keys")
...
### CDN security keys[​](#cdn-security-keys "Direct link to CDN security keys")
...
V2 webhooks also support additional event types for the new resource types (e.g. `engine.deploying`, `engine.playing`, `ingest.created`, `distribution.created`, etc.). See the [API reference](/documentation/pr-preview/pr-690/theolive/api.md) for the full list.

## Pagination[​](#pagination "Direct link to Pagination")

V1 used page-based pagination (`page` parameter) or `startingAfter` with an ID. V2 uses [cursor-based pagination](/documentation/pr-preview/pr-690/theolive/api/pagination.md) across all list endpoints:
...
## Removed V1 endpoints[​](#removed-v1-endpoints "Direct link to Removed V1 endpoints")
...
## Starting and stopping[​](#starting-and-stopping "Direct link to Starting and stopping")
