...
A media distribution is an object defining security settings for a subset of media assets on your account.
...
To view a media asset associated with this distribution, simply use the `url` field from the [Read Media Asset](/millicast/api/media-assets-read-media-asset.api.mdx) response.
