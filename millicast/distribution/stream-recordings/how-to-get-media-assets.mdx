...
For fine-grain control over access to media assets, use media distributions and the `url` media asset field.

A media distribution in an object defining security settings for a subset of media assets on your account. 
...
1. Specify the `mediaDistributionId` when creating a clip using the [Create Media Asset](/millicast/api/media-assets-create-media-asset.api.mdx) endpoint.
...
3. Set a default media distribution for each type of media using the Update Default Media Distribution Settings endpoint. Any new media assets created on your account will be associated with the default media distribution.

### Secured Playback
[Add a security key](/millicast/api/media-distributions-create-security-key.api.mdx) to your media distribution, noting the `id` in the response. 
Keep a record of your secret key since you will not be able to retrieve it again via the OptiView APIs.
Any media assets associated with a media distribution with a security key cannot be viewed without a valid JWT.

Create a JWT with a payload including the `id` of your media distribution security key and an expiry time. Use your secret key to sign the JWT.
This JWT can be used to view any media asset associated with the media distribution.
...
### Unsecured Playback
Media assets can be viewed without a token if they are associated with an "unsecured" media distribution. That is, one without any security keys. 
The base download `url` can be used without a token to view any media asset associated with an unsecured media distribution.
