...
Re-streaming is deduplicated **per publish token on each publisher node**. Only one stream per publish token can actively re-stream on a given node at a time. If a new feed starts publishing with the same token that is already re-streaming, the existing re-stream is stopped and the new feed takes over.

This means that when multiple streams share the same publish token, only the most recently connected feed will re-stream. To prevent this, add `&norestream` to the publishing URLs of any feeds that should not re-stream.

Note that this dedup is **node-local** — if feeds using the same token connect to different publisher nodes, each node will independently re-stream its feed, which may result in duplicate re-streams to the same destination.
:::
