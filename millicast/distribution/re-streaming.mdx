...
- If two ingests are routed to the same node, one re-stream may be stopped and replaced by the other.
- If they are routed to different nodes, both may attempt to re-stream to the same endpoint simultaneously, causing errors or corrupted statistics at the re-stream destination.

Because of this, **re-streaming multiple ingests with the same publish token is not supported**. To re-stream different ingests independently, use a **separate publish token** for each ingest. If multiple ingests must share the same publish token, add `&norestream` to the publishing URLs of any ingests that should not re-stream.
