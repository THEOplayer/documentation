...
- [WebRTC](/millicast/broadcast/webrtc-whip.mdx), an internet transfer protocol that supports video codecs H.264, H.265, VP8, VP9, AV1, and the Opus audio codec. Broad support is made possible through implementations of **WebRTC HTTP Ingest Protocol (WHIP)**.
- [SRT](/millicast/broadcast/using-srt.mdx), a video transfer protocol that can be transmuxed to WebRTC via the Dolby.io CDN and supports H.264 video and AAC audio.
- [RTMP](/millicast/broadcast/using-rtmp-and-rtmps.mdx) **and RTMPS**, internet transfer protocols that can be transmuxed to WebRTC via the Dolby.io CDN that supports only the H.264 video codec.
...
| :---------------------------------------------------------------------------------------- | :--------------------------------------------- | :-------------------------------------------------- |
| [Osprey](/millicast/hardware-encoders/using-osprey-talon-whip-hardware-encoder.mdx) Talon | RTMP, RTMPS, RTSP, HLS, RTP, SRT, WebRTC, WHIP | H.264 (AVC), H.265 (HEVC), AAC                      |
| [Teradek](/millicast/hardware-encoders/broadcasting-teradek-vidiu.mdx) Wave               | RTMP, RTMPS, RTSP, SRT, HLS, NDI               | H.264 (AVC), H.265 (HEVC), AAC                      |
| [Videon](/millicast/hardware-encoders/videon.mdx) EdgeCaster                              | RTMP, RTMPs, SRT, HLS                          | H.264 (AVC), H.265 (HEVC), VP9, AAC                 |
| [Haivision](/millicast/hardware-encoders/using-haivision-kb-encoder.mdx) KB Encoder       | RTMP, RTMPS, RTSP, HLS, SRT                    | H.264 (AVC), H.265 (HEVC), AAC                      |
| [FlowCaster](/millicast/software-encoders/using-whip-with-flowcaster.mdx)                 | RTMP, RTMPs, WHIP, NDI, HLS, SRT, WebRTC       | H.264 (AVC), H.265 (HEVC)                           |
| [FFmpeg](/millicast/software-encoders/using-ffmpeg.mdx)                                   | RTMP, RTMPs, RTSP, HLS                         | H.264 (AVC), H.265 (HEVC), VP9, AV1, AAC            |
| [GStreamer](/millicast/software-encoders/using-whip-with-gstreamer.mdx)                   | RTMP, RTMPs, RTSP, SRT, HLS, WHIP              | H.264 (AVC), H.265 (HEVC), VP9, AV1, AAC            |
| [OBS](/millicast/software-encoders/obs/index.mdx)                                         | WebRTC, RTMP, RTMPs, NDI, HLS, SRT, RTSP       | H.264 (AVC), H.265 (HEVC), VP9, AAC                 |
| [LiveU](/millicast/software-encoders/liveu-studio-using-whip.mdx)                         | RTMP, RTMPs, HLS, RTSP, NDI                    | H.264 (AVC), H.265 (HEVC), VP9, AAC                 |
| [vMix](/millicast/software-encoders/broadcasting-with-vmix.mdx)                           | RTMP, RTMPs, RTSP, NDI, SRT, WebRTC            | H.264 (AVC), H.265 (HEVC), ProRes, DNxHD/DNxHR, AAC |
...
[How-to use GStreamer with WHIP](/millicast/software-encoders/using-whip-with-gstreamer.mdx)<br/>
...
[How-to use FlowCaster with WHIP](/millicast/software-encoders/using-whip-with-flowcaster.mdx)<br/>
**FlowCaster** software provides a secure connection to provide IP streaming for editors and remote collaboration.

[How-to use LiveU Studio with WHIP](/millicast/software-encoders/liveu-studio-using-whip.mdx)<br/>
...
[How-to Setup an Osprey Talon for WHIP](/millicast/hardware-encoders/using-osprey-talon-whip-hardware-encoder.mdx)<br/>
...
The **Secure Reliable Transport ([SRT](/millicast/broadcast/using-srt.mdx))** is an open-source protocol that uses an intelligent packet retransmission mechanism on otp of a UDP data flow along with AES-128 and 256-bit encryption. Dolby is part of the _SRT Alliance_, an organization to promote the technology for optimizing streaming performance.

[How-to setup a Dolby.io SRT Stream](/millicast/broadcast/using-srt.mdx)<br/>
The Dolby.io dashboard is used to configure the publish path and stream id for an incoming SRT media stream.

[How-to use OBS with SRT](/millicast/software-encoders/obs/index.mdx)<br/>
...
**Real-time Messaging Protocol ([RTMP](/millicast/broadcast/using-rtmp-and-rtmps.mdx))** is well-established in many hardware and software applications. With many years of use, RTMP is a good option for compatibility while sacrificing some latency in order to handle transmuxing the incoming stream to WebRTC.

[How-to setup a Dolby.io RTMP Stream](/millicast/broadcast/using-rtmp-and-rtmps.mdx)<br/>
The Dolby.io dashboard is used to configure the publish path and stream id for an incoming RTMP or RTMPs media stream.

[How-to use FFmpeg with RTMP](/millicast/software-encoders/using-ffmpeg.mdx)<br/>
...
[How-to use vMix with RTMP](/millicast/software-encoders/broadcasting-with-vmix.mdx)<br/>
...
[How-to Setup a Haivision KB encoder for RTMP](/millicast/hardware-encoders/using-haivision-kb-encoder.mdx)<br/>
...
[How-to Setup a Teradek VidiU Go for RTMP](/millicast/hardware-encoders/broadcasting-teradek-vidiu.mdx)<br/>
