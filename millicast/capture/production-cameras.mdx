...
- **Does my camera have a built-in encoder, or will I need to connect to one?** The camera does not have a built-in encoder, so we'll use a [Videon EdgeCaster](/millicast/broadcast/hardware-encoders/videon.mdx).
- **Am I planning to broadcast with [RTMP, RTMPs](/millicast/broadcast/rtmp-and-rtmps.mdx), [SRT](/millicast/broadcast/srt.mdx), or [WebRTC](/millicast/broadcast/hardware-encoders/osprey.mdx)?** Videon supports RTMP output, so we'll use that.
