...
If you click the `Customize` button in the Playback Dashboard you'll see there are lots of options you can use to include or exclude playback behaviors. The [How to Broadcast in Dashboard](/millicast/streaming-dashboard/how-to-broadcast-in-dashboard.mdx) page goes through these options in more detail.
...
To dynamically generate a viewer you should use the [Web SDK](/millicast/client-sdks/web/sdk/index.mdx) in your application. You can do this by adding a `<script>` element in the `<head>` section to include the SDK from a hosted location such as a [CDN](https://www.jsdelivr.com/package/npm/@millicast/sdk).
...
Review the [Web SDK](/millicast/client-sdks/web/sdk/index.mdx) documentation for additional installation methods such as with `npm`. The Streaming API used to be known as Millicast so the SDK uses that name when importing the library.
...
The [Web SDK](/millicast/client-sdks/web/sdk/index.mdx) is pulled in with a `<script>` element from a CDN in the same way as is done for the playback-app.
...
Continue by exploring the [Streaming Web SDK](/millicast/client-sdks/web/sdk/index.mdx) documentation for a guide on other functions as you continue building your own project.
