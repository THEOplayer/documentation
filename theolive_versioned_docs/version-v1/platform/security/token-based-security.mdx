...
Please refer to the [Enable token security for alias](../../api/channels/enable-token-security-for-alias.api.mdx) and [Disable token security for alias](../../api/channels/disable-token-security-for-alias.api.mdx) API endpoints to manage the token security settings for an alias.
