...
You can enable geo-blocking on a channel by [updating](../../api/channels/update-channel.api.mdx) the `geoBlocking` object within `publicationConfig`. The `mode` of geo-blocking can also be configured:
...
Geo-blocking can also be done on a [channel alias](../multi-channel.md) using the specified channel alias [endpoint](../../api/channels/update-channel-alias.api.mdx) or via the console.
