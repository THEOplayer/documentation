...
| [Integrations](../integrations/google.mdx) | Manage channel-level delivery integrations.                           |
...
- **`wallclock`** — breaks are scheduled based on the stream's wallclock timing: the break's start (`startWallclock`, a UTC timestamp) is matched against the wallclock timeline carried by the stream itself. Use this when the stream carries wallclock timing: in HLS this comes from `EXT-X-PROGRAM-DATE-TIME` tags, in DASH from the MPD's `availabilityStartTime` combined with the segment timeline (optionally synchronized through a `UTCTiming` element).
