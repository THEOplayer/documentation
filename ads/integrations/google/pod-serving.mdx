...
Line item types define priority tiers, which control which ads win when multiple line items target the same ad unit. Typical tiers are:

- **Sponsorship** — highest priority, for exclusive or takeover deals; delivers a percentage of matching requests.
- **Standard (premium)** — guaranteed impression goals with high priority.
- **Standard (normal) and lower tiers** — non-guaranteed inventory such as price priority, network, or house line items that fill remaining requests.

By targeting your live stream's ad unit from line items in different tiers, you control which advertiser fills each break.
...
Players can extend these parameters: the OptiView player SDKs accept `adTagParameters` on the ad description (`TheoAdDescription`), and the player automatically adds parameters such as `theoads_slot`. When the same parameter is set both on the break and on the player, the player-side value wins. See [Ad tag parameters](../../how-to-guides/ad-tag-parameters.md) for the player-side API on each platform.
