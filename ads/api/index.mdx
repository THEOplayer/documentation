...
```bash
ADS_OPENAPI_SPEC_URL=/path/to/optiview-ads/openapi.json npm run gen-api-docs
ADS_OPENAPI_REF=commit-or-branch npm run gen-api-docs
```

Private GitHub fetches use `ADS_OPENAPI_TOKEN`, `GITHUB_TOKEN`, or `GH_TOKEN`. Change the pinned commit in `scripts/fetch-ads-openapi.mjs` when intentionally refreshing the published contract, regenerate the pages, and review the generated reference.
