...
When using the [Open Video UI for Android](/theoplayer/how-to-guides/android/ui/open-video-ui/), you need to create and add the `TheoAdsIntegration` before creating your `DefaultUI` or `UIController`. You can then create an OptiView Ads-enabled source and set it as `player.source`:
