...
Use a combination of one vertical (`top` or `bottom`) and one horizontal (`left` or `right`) property to anchor the overlay.
...
You can add multiple overlays to a single engine. For example, a logo in the bottom-right and a "LIVE" badge in the top-left:
...
```json
{
  "overlays": [
    ...
    {
      "url": "https://example.com/live-badge.png",
      "position": {
        "top": 10,
        "left": 10
      },
...
- To remove overlays, update the engine with an empty `overlays` array.
