...
THEOlive lives by the "API first" mantra: each action can be performed using API calls. Each possible call can be found at our [API Reference](https://developers.theo.live/reference/create-channel). However, to make things easier for non-developers, we built a management console on top of these API calls which makes it straightforward to perform the basic actions. To be able to create and use a THEOlive channel, you should register for an account and get a token to perform [authenticated requests](../getting-started/authorization) to our API.
