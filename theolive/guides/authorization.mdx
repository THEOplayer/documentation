...
For example, assume the token has value `my-token` and the secret has value `my-secret`:
    1. Combine them like this: `my-token:my-secret`.
    2. Encode this value using base64: `bXktdG9rZW46bXktc2VjcmV0`
    3. Pass the full Authorization header (with `Basic` included) in your REST calls: `Authorization: Basic bXktdG9rZW46bXktc2VjcmV0`
