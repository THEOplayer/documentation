...
Metadata insertion is applied at the channel level. Any engine within the channel will receive the same metadata messages so in an active-active setup, both engines will get the metadata. At this time it is not possible to disable metadata on an individual engine within a channel.
