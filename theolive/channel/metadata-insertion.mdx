...
Metadata insertion lets you deliver custom timed data to viewers through the OptiView Live pipeline. The metadata is carried alongside the live stream so it arrives at the player synchronized with the video. Use it for cuepoints, score updates, statistics, or any other event-driven information you want to deliver to viewers in real time.
...
- **Push via the API** — at any time, POST a binary payload to a UUID of your choosing. The platform attaches it to the live stream as soon as it is received.
