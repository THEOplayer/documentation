...
THEOlive offers the option to enable JWT token security for channels (including channel aliases). This can be interesting if you only want valid users to access your stream.
...
When not passing a bearer token for a secured channel, the request will be rejected. If the channel is not secured, you may still pass a token but it is not required.
