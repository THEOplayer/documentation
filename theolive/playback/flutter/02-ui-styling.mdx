...
The THEOplayer Flutter SDK used for THEOlive playback is currently completely [`chromeless`](/theoplayer/flutter/guides/limitations/), providing no default UI for the Player.
...
You can also copy and paste the `helpers` folder, which contains a `utils.dart` file that will allow you to update the example Player's color, into your project. Remove the default `theoYellow`
color variable and add your own. Note that you will also have to update the `theoYellow` variable references within the `player_widgets` folder to your own.

Once done, your Player will appear in your application like this:

![Default Flutter Player UI](../../assets/img/flutter-player-ui.png)
...
```dart
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Align(
          // add your widget to the top of the player's container
          alignment: Alignment.topCenter,
          // insert your widget into the child property below
          // child: {Add your widget here}
        ),
        Expanded(child: Container()),
        Align(
          // add your widget to the center of the player's container
          alignment: Alignment.center,
          child: PlayButtonWidget(player: player)
        ),
        Expanded(
          child: Align(
            // add your widget to the bottom of the player's container
...
class PosterWidget extends StatefulWidget {
  const PosterWidget({
    super.key,
    // require a player instance be passed in
    required this.player,
    // require a poster URL be passed in to
...
class _PosterWidgetState extends State<PosterWidget> {
  ...
  void initState() {
    super.initState();
    // use the 'played' property on the player instance to check if the stream has ever played. The played
    // property will return an empty array if the stream has not been started since the page loaded.
    hasPlayed = widget.player.played.isNotEmpty;

    // optionally you can listen for the player events to update the hasPlayed variable
  ...
  Widget build(BuildContext context) {
    ...
    return !hasPlayed
        ? Container(
            ...
            // note - you will also have to add your image assets to the pubspec.yaml file to use them
...
you will need to use a [`Stack`](https://api.flutter.dev/flutter/widgets/Stack-class.html) to overlap widgets in the
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        ...
        // ensure poster appears below play button, but above the other widgets that will only be displayed upon the player starting
        // pass the poster image URL to the poster widget if using an image hosted publically on the internet.
        Align(
          alignment: Alignment.center,
          child: PosterWidget(
...
![THEOlive Flutter Player Poster](../../assets/img/flutter_theolive_poster_example.png)
...
Within the `player_widgets` folder create a `error_handling_widget` file that will contain a custom widget for handling errors.
...
class ErrorHandlingWidget extends StatefulWidget {
  const ErrorHandlingWidget({
    super.key,
    required this.player,
    // optional custom error message string can be passed to the widget to display for all errors
    this.errorMessage,
  });
  ...
  Widget build(BuildContext context) {
    return streamError
        ? Container(
            ...
            // to use a locally hosted asset folder image
            //   image: AssetImage(
            //       'assets/images/background.png'),

            // or to use a remotely hosted image
            ...
            child: Text(
                // use custom message passed to widget or a predefined message
                // widget.errorMessage ?? 'Your stream is currently unavailable',

                // return a specific error message from the player 'error' property or a predefined message
                widget.player.error ?? 'Your stream is currently unavailable',
...
![Flutter Error Handling Example](../../assets/img/flutter_error_handling_example.png)
...
You can use the `play_button-widget.dart` file for an example of how to conditionally render a widget based on the player's state.
...
To have the logo overlay appear on the bottom right of the player without disrupting the layout, you add the following code to the `player_ui_widget.dart` file's build method return:
...
![Custom Flutter Logo Overlay](../../assets/img/theolive_flutter_logo_overlay_example.png)

Note: the logo overlay can be customized to show when different Player events occur by creating event
listeners and changing the `logo_overlay_widget` file's state logic.
