...
- Lastly, set "Frames per second" to the same value used in the channel ingest config and set "Average bit rate" to match the max bitrate of the channel preset. See [Stream configuration](../../stream-configuration/stream-configuration.mdx) for more details.
