...
Each request that carries a token is checked on its own. On a channel without token-based security there is no key to verify who issued the token, so this is not a way to control access or to limit how long someone can watch — enable token-based security if you need that.
::::
