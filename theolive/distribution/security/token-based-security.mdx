...
When a token is rejected, an HTTP `403` on its own is hard to diagnose: you know the token you issued, but not the country, region, DMA code or device type the CDN resolved for that viewer, and those are exactly the values the [geo](#geo-restrictions) and [device](#device-type-mapping) restrictions were evaluated against.

Every rejection caused by the token therefore carries an `x-optiview-viewer` header describing how the CDN sees the viewer. The value is a comma-separated list of `<attribute>=<value>` pairs:

| Attribute | Description                                                                       | Example     |
| --------- | --------------------------------------------------------------------------------- | ----------- |
| `cc`      | Country, in [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1) format | `cc=US`     |
| `rgn`     | Region, in [ISO 3166-2](https://en.wikipedia.org/wiki/ISO_3166-2) format          | `rgn=US-CA` |
| `dma`     | DMA (Designated Market Area) code                                                 | `dma=803`   |
| `hw`      | Device type, as described under [device type mapping](#device-type-mapping)       | `hw=mobile` |

For example, a token restricted with `"geo": { "cc": ["BE"] }` presented by a viewer in California is rejected with:

```http
HTTP/2 403
x-optiview-viewer: cc=US,rgn=US-CA,dma=803,hw=desktop
```
...
- The values describe the **viewer**, not the token, and not the rejection. Every rejection caused by the token reports the same attributes, whether the token was expired, malformed, signed with the wrong key, or simply restricted to a different region, so the header never tells you which restriction failed. Compare it against the token to work that out.
- Only attributes the CDN could determine are listed. A viewer in a country without region subdivisions, or outside the United States and Canada (where regions and DMA codes are supported), yields `cc` only.
- The header is exposed through `Access-Control-Expose-Headers`, so a browser-based player can read it on the `403`.

Rejections that are not caused by the token carry no viewer attributes: the distribution's own [geo-blocking](./geo-blocking.md), [IP blocking](./ip-blocking.md) and [referrer blocking](./referrer-blocking.mdx) rules, and a distribution that is not scheduled to be live, all return a plain `403`.
