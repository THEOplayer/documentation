...
- `adURITemplate`: The Ad Decisioning Template URI which is to be used for ad requests, default value `theo-ads://dai.break/?d=[duration]`.
...
The Signaling Service will not add the origin URL into the manifest in the case of a relative URL.
However, segment URLs in the media playlists should be absolute URLs.
Segment requests don't need to pass through the Signaling Service but should be handled directly on their origin.
